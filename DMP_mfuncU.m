function [out1,out2,out3] = DMP_mfunc(flag,s,x,Eh,e,U_ss)
...
switch flag
    ...
    case 'f';
        ...
        out1      = zeros(n,dx);
        out1(:,1) = Eh(:,1)-c./(delta*q);
        out1(:,2) = Eh(:,2)-U+(z+c.*(beta*theta)/(1-beta))/U_ss;
        
        % derivative with respect to x
        out2        = zeros(n,dx,dx);
        out2(:,1,1) = theta.*c.*q1./(delta*q.^2);    % ln(theta)
        out2(:,1,2) = zeros(n,1);                    % U
        out2(:,2,1) = c.*(beta*theta)/(1-beta)/U_ss; % ln(theta)
        out2(:,2,2) = -ones(n,1);                    % U
